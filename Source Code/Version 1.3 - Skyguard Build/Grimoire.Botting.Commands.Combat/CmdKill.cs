...
namespace Grimoire.Botting.Commands.Combat
{
    public class CmdKill : IBotCommand
    {
        ...
        public async Task Execute(IBotEngine instance)
        {
            ...
            await instance.WaitUntil(() => World.IsMonsterAvailable(Monster), null, 5);
                ...
                if (ClassIndex != -1)
                {
                    ...
                    if (s.Type == Skill.SkillType.Safe)
                    {
                        if (s.SafeMp)
                        {
                            if (s.ComparisonType)
                            {
                                if (Player.Mana / (double)Player.ManaMax * 100.0 >= s.SafePercentage)
                                {
                        ...
                        else if (s.SafeHp)
                        {
                            if (s.ComparisonType)
                            {
                                if (Player.Health / (double)Player.HealthMax * 100.0 >= s.SafePercentage)
                                {
                                    Player.UseSkill(s.Index);
                                }
                            }
